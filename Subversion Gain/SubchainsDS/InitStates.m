global alphaPower gammaRatio maxForkLen maxDw minDw numOfStates mtimes DSR cfN
global POut RsOut
...
baseStateHb = st2stnum(0, 1, -1, irrelevant, 0, false);
baseStateHi = st2stnum(0, 0, -1, irrelevant, 0, false);
baseStateAb = st2stnum(1, 0, 1, irrelevant, 0, false);
baseStateAi = st2stnum(0, 0, 1, irrelevant, 0, false);
...
for i = 1:numOfStates
    ...
    %     tip of the chain
    [a, h, dw, fork, last, matched] = stnum2st(i);
    if h >= cfN && matched==false
        DSRcur = DSR*(h-cfN+1);
    else
        DSRcur = 0;
    end
    ...
    if dw>0
        ...
        P{override}.c(i) = st2stnum(aLeft, 1, lastLenLeft-1, relevant, newLast, false);
        P{override}.P(i) = Phb;
        P{override}.Rs(i) = a-aLeft+DSRcur;
        % next is honest inblock
        P{override}.ii = P{override}.ii+1;
        P{override}.r(P{override}.ii) = i;
        P{override}.c(P{override}.ii) = st2stnum(aLeft, 0, lastLenLeft-1, relevant, newLast, false);
        P{override}.P(P{override}.ii) = Phi;
        P{override}.Rs(P{override}.ii) = a-aLeft+DSRcur;
        % next is selfish block
        P{override}.ii = P{override}.ii+1;
        P{override}.r(P{override}.ii) = i;
        P{override}.c(P{override}.ii) = st2stnum(aLeft+1, 0, lastLenLeft+1, irrelevant, newLast*2+1, false);
        P{override}.P(P{override}.ii) = Pab;
        P{override}.Rs(P{override}.ii) = a-aLeft+DSRcur;
        % next is selfish inblock
        P{override}.ii = P{override}.ii+1;
        P{override}.r(P{override}.ii) = i;
        P{override}.c(P{override}.ii) = st2stnum(aLeft, 0, lastLenLeft+1, irrelevant, newLast*2, false);
    ...
    if a < maxForkLen && h < maxForkLen && dw<maxDw && dw>minDw
        if fork ~= active
            % next is honest block
            P{wait}.c(i) = st2stnum(a, h+1, dw-1, relevant, last, matched);
            P{wait}.P(i) = Phb;
            % next is honest inblock
            P{wait}.ii = P{wait}.ii+1;
            P{wait}.r(P{wait}.ii) = i;
            P{wait}.c(P{wait}.ii) = st2stnum(a, h, dw-1, relevant, last, matched);
            P{wait}.P(P{wait}.ii) = Phi;
            % next is selfish block
            P{wait}.ii = P{wait}.ii+1;
            P{wait}.r(P{wait}.ii) = i;
            P{wait}.c(P{wait}.ii) = st2stnum(a+1, h, dw+1, irrelevant, last*2+1, matched);
            P{wait}.P(P{wait}.ii) = Pab;
            % next is selfish inblock
            P{wait}.ii = P{wait}.ii+1;
            P{wait}.r(P{wait}.ii) = i;
            P{wait}.c(P{wait}.ii) = st2stnum(a, h, dw+1, irrelevant, last*2, matched);
        ...
        elseif fork == active
            ...
            P{wait}.ii = P{wait}.ii+1;
            P{wait}.r(P{wait}.ii) = i;
            P{wait}.c(P{wait}.ii) = st2stnum(blocksInLast, 1, dw-1, relevant, last, false);
            P{wait}.P(P{wait}.ii) = Phb*gammaRatio;
			P{wait}.Rs(P{wait}.ii) = a-blocksInLast+DSRcur;
            % honest inblock on selfish chain
            P{wait}.ii = P{wait}.ii+1;
            P{wait}.r(P{wait}.ii) = i;
            P{wait}.c(P{wait}.ii) = st2stnum(blocksInLast, 0, dw-1, relevant, last, false);
            P{wait}.P(P{wait}.ii) = Phi*gammaRatio;
			P{wait}.Rs(P{wait}.ii) = a-blocksInLast+DSRcur;
            % selfish block on selfish chain
            P{wait}.ii = P{wait}.ii+1;
            P{wait}.r(P{wait}.ii) = i;
            P{wait}.c(P{wait}.ii) = st2stnum(a+1, h, dw+1, active, last*2+1, matched);
            P{wait}.P(P{wait}.ii) = Pab;
            % selfish inblock on selfish chain
            P{wait}.ii = P{wait}.ii+1;
            P{wait}.r(P{wait}.ii) = i;
            P{wait}.c(P{wait}.ii) = st2stnum(a, h, dw+1, active, last*2, matched);
            P{wait}.P(P{wait}.ii) = Pai;
        end
    end
    ...
    if fork == relevant && dw>=0 && dw<=3 && a+1 <= maxForkLen && h+1 <= maxForkLen
        ...
        % honest block on honest chain
        P{match}.c(i) = st2stnum(a, h+1, dw-1, relevant, last, newMatched);
        P{match}.P(i) = Phb*(1-gammaRatio);
        % honest inblock on honest chain
        P{match}.ii = P{match}.ii+1;
        P{match}.r(P{match}.ii) = i;
        P{match}.c(P{match}.ii) = st2stnum(a, h, dw-1, relevant, last, newMatched);
        P{match}.P(P{match}.ii) = Phi*(1-gammaRatio);
        % honest block on selfish chain
        P{match}.ii = P{match}.ii+1;
        P{match}.r(P{match}.ii) = i;
        P{match}.c(P{match}.ii) = st2stnum(blocksInLast, 1, dw-1, relevant, last, false);
        P{match}.P(P{match}.ii) = Phb*gammaRatio;
		P{match}.Rs(P{match}.ii) = a-blocksInLast+DSRcur;
        % honest inblock on selfish chain
        P{match}.ii = P{match}.ii+1;
        P{match}.r(P{match}.ii) = i;
        P{match}.c(P{match}.ii) = st2stnum(blocksInLast, 0, dw-1, relevant, last, false);
        P{match}.P(P{match}.ii) = Phi*gammaRatio;
		P{match}.Rs(P{match}.ii) = a-blocksInLast+DSRcur;
        % selfish block on selfish chain
        P{match}.ii = P{match}.ii+1;
        P{match}.r(P{match}.ii) = i;
        P{match}.c(P{match}.ii) = st2stnum(a+1, h, dw+1, active, last*2+1, newMatched);
        P{match}.P(P{match}.ii) = Pab;
        % selfish inblock on selfish chain
        P{match}.ii = P{match}.ii+1;
        P{match}.r(P{match}.ii) = i;
        P{match}.c(P{match}.ii) = st2stnum(a, h, dw+1, active, last*2, newMatched);
